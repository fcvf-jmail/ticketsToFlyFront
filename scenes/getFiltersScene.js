...
const filterKeyboard = [
    ...
    [{text: "🔙 Назад", callback_data: "backToPreviousScene"}],
    [cancelButton]
]

const getFiltersScene = new Scenes.BaseScene("getFiltersScene")

getFiltersScene.enter(async ctx => {
...
getFiltersScene.action("back", async ctx => {
    return await ctx.scene.enter("getFiltersScene", ctx.scene.session.state)
})

getFiltersScene.action("backToPreviousScene", async ctx => {
    return await ctx.scene.enter("getDateIntervalScene", ctx.scene.session.state)
})

getFiltersScene.action("cancelSubscription", async ctx => {
    return await cancelSubscription(ctx)
})

getFiltersScene.action("setMaxPrice", async ctx => {
...
getFiltersScene.action("setMinBaggage", async ctx => {
...
getFiltersScene.action("setMinHandBaggage", async ctx => {
...
getFiltersScene.action("setTransfers", async ctx => {
...
getFiltersScene.action("setFlightTime", async ctx => {
...
getFiltersScene.action("finishSubscription", async ctx => {
    var hourWord = "час"
...
getFiltersScene.on(message("text"), async ctx => {
...
async function sendSubscriptionToDb(subscription) {
    ...
    await axios.post("http://localhost:1728/api/Subscription", filteredSubscription)
}

module.exports = { getFiltersScene };
